...
버그 수정과 재현성 개선을 환영합니다. 실제 데이터셋이나 GPU가 없어도 합성 데이터로 전체
파이프라인을 검증할 수 있어야 합니다.
...
스모크 테스트는 작은 EventHDR H5와 EventAid-R ZIP을 임시로 생성하므로 공식 데이터셋을
내려받지 않습니다. Windows와 Linux의 Python 3.10, 3.11, 3.12 조합은 GitHub Actions에서
동일한 설치 및 검증 절차로 검사합니다.

## Pull request 원칙

- 한 PR에는 가능한 한 하나의 논리적 변경만 포함합니다.
- 모델, 손실 함수, 그래프 생성 또는 데이터 정렬을 변경하면 근거와 예상 영향을 설명합니다.
- 새로운 기능에는 최소 단위 테스트나 합성 데이터 재현 절차를 추가합니다.
...
재현에 필요한 메타데이터와 작은 합성 fixture만 저장하세요. 버그 보고에 실제 데이터 일부가
필요하다면 먼저 배포 라이선스와 개인 정보 포함 여부를 확인하고, 가능하면 이를 대신하는 최소
합성 예제를 제공하세요.
